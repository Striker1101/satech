(function ($) {
    ...
    function gsap_rs_split_text() {
        setTimeout(function () {
            var splitTextElements = $(
                "body:not(.rtl) div.is-page .rs-split-text-enable"
            );
    ...
    function gsap_rs_marqueeSlider() {
        ...
        if (rs_Marquee_Slider.length) {
            ...
            Array.from(rs_Marquee_Slider).forEach((element) => {
                let directionClass = element.classList.contains("left")
                    ? "left"
                    : "right";
    ...
    function gsap_rs_scrollRotate() {
        ...
        document
            .querySelectorAll(".gsap-rotate, .gsap-rotate-normal")
            .forEach((element, index) => {
                let rotationSpeed = 0.2;
                let rotationClass = element.className.match(/to-(\d+)/);
                let rotationValue = rotationClass
                    ? parseInt(rotationClass[1])
                    : 360;
                gsap.to(element, {
        ...
        document
            .querySelectorAll(".gsap-rotate-anti")
            .forEach((element, index) => {
                let rotationSpeed = 0.2;
                let rotationClass = element.className.match(/to-(\d+)/);
                let rotationValue = rotationClass
    ...
    function gsap_rs_MoveOnScroll() {
        gsap.registerPlugin(ScrollTrigger);
        document
            .querySelectorAll(".gsap-move, .gsap-move-yes")
            .forEach((element, index) => {
                let moveDistance = extractMoveDistanceAnadDirectionFromClass(
                    element.className
                );
                let startPercentage = extractStartPercentageFromClass(
                    element.className,
                    20
                );
                let moveProperties = {};
    ...
    function gsap_rs_custom_cursor() {
        ...
        if (cursorBall) {
            ...
            // Common Area
            $(
                "a, button, input[type=submit], .slick-arrow, .slick-dots li button, .player-btn-comn, .revslider-initialised rs-arrow, .search_icons i, .rs-mnt, .rs-yrs, .tp-bullet, .rsoffwrap-close, .pricing-switcher-wrapper span, .rs-work-process-scroll-btn .btn-inner"
    ...
    function gsap_rs_image_reveal() {
        ...
        document
            .querySelectorAll(".rs-image.scroll_reveal")
            .forEach((wrapper) => {
                let image = wrapper.querySelector(
                    ".rs-image.scroll_reveal img"
                );
                let directionClass = wrapper.classList.contains("reveal_left")
                    ? "reveal_left"
                    : "reveal_right";

                let reveal_trigger = gsap
                    .timeline({
                        scrollTrigger: {
                            trigger: wrapper,
                            toggleActions: "restart pause resume reverse",
                ...
                if (directionClass === "reveal_left") {
                    reveal_trigger
                        .from(wrapper, 1, { xPercent: -50, ease: Power2.out })
                        .from(image, 1, {
                            xPercent: 50,
                            scale: 1.3,
                            delay: -1,
    ...
    function gsap_fixed_elements() {
        ...
        const containers = document.querySelectorAll(
            ".gsap-fixed, .gsap-fixed-yes"
        );
        containers.forEach((container) => {
            const elements = container.querySelectorAll(
                ".gsap-fixed > .e-con-inner > *, .gsap-fixed-yes > *"
            );
            elements.forEach((element, index) => {
                const startClass = Array.from(container.classList).find(
                    (className) => className.startsWith("start-")
                );
                const startOffset = startClass
                    ? parseInt(startClass.split("-")[1])
                    : 40;
        ...
        wrappers.forEach((wrapper) => {
            ...
            function updatePosition(e) {
                ...
                const mouseX = Math.min(
                    Math.max(e.clientX, bounds.left),
                    bounds.right
                );
                const mouseY = Math.min(
                    Math.max(e.clientY, bounds.top),
                    bounds.bottom
                );
