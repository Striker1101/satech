...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        if (sections.length) {
            sections.forEach((section) => {
                const thresholdClass = [...section.classList].find((cls) =>
                    cls.startsWith("threshold-")
                );
                const threshold = thresholdClass
                    ? parseFloat(thresholdClass.split("-")[1])
                    : 0.4;
                ...
                obj.find(settings.classes.skillBarBarcir).animate(
                    {
                        strokeDashoffset:
                            strokdash -
                            (strokdash * parseInt(obj.attr("data-percent"))) /
                                100,
                    },
                    settings.speed
                );
    ...
    var portfolioBlurImg = $(
        ".rs-portfolios-grid.glass-effect-yes .portfolio-item"
    );
    ...
    rsPageBody.on(
        ...
        function (e) {
            ...
            if (
                ("undefined" !== typeof validUrl.valid &&
                    validUrl.valid === false) ||
                validUrl === false
            ) {
                e.preventDefault();
