...
var ddbeforeandafter = (function ($) {
    ...
    function ddbeforeandafter(options) {
        ...
        $container.hover(
            ...
            function () {
                s._draghidetimer = setTimeout(function () {
                    $dragbar.stop().animate({ opacity: 0.3 });
            ...
            $(document).bind(mousemoveevtstr, function (e) {
                ...
                var newx =
                    dx < 0
                        ...
                        : Math.min(
                              dimensions.containerw - dimensions.dragbarw,
                              initialx + dx
                          ); //Set horizonal bonds. dx>0 indicates drag right versus
                $dragbar.css({ left: newx });
                $before.css({ width: newx });
        ...
        $(window).bind("resize", function (e) {
            ...
            $before
                .find("img:eq(0)")
                .css({ width: dimensions.containerw, height: "auto" });
            $after
                .find("img:eq(0)")
                .css({ width: dimensions.containerw, height: "auto" });
            thisobj.unveil(
    ...
    ddbeforeandafter.prototype = {
        _centerelement: function ($parent, $target, offsetx, offsety) {
            ...
            $target.css({
                left:
                    dimensions.parentw / 2 -
                    dimensions.targetw / 2 +
                    (offsetx || 0),
                top:
                    dimensions.parenth / 2 -
                    dimensions.targeth / 2 +
                    (offsety || 0),
            });
        },
        ...
        _init: function (initoptions) {
            ...
            var $beforeimage = this.$before
                .css({ opacity: 0.5 })
                .find("img:eq(0)")
                .css({ width: this.dimensions.containerw, height: "auto" });
            var $afterimage = this.$after
                .css({ opacity: 0 })
                .find("img:eq(0)")
                .css({ width: this.dimensions.containerw, height: "auto" });
        ...
        unveil: function (amt, dur, timingfunc) {
            ...
            var dur =
                (typeof dur != "undefined" ? dur : s.revealduration) + "s";
            var timingfunction = timingfunc || s.defaultTimingFunction;
            amt = 100 - amt;
            var newx =
                ((dimensions.containerw - dimensions.dragbarw) * amt) / 100;
            $dragbar.add($before).css({
