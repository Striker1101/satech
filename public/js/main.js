 ...
 * --------------------------------------------------------------------
 *
 * Template : Abuild â€“ Industrial WordPress Theme
 * Author : RSTheme
...
(function ($) {
    ...
    if ($("body").hasClass("page-template-page-single2")) {
        $("body").addClass(
            "page-template-page-single page-template-page-single-php"
        );
    }
    //preloader
    ...
    $(".wpcf7-form-control.wpcf7-select").wrapAll(
        "<em class='select-full'></em>"
    );
    $(".rs-heading .description p:empty").remove();
    $(".woocommerce-product-gallery, .summary.entry-summary").wrapAll(
        "<div class='rs-wrap-summery'></div>"
    );
    $(".form-row.form-row-first, .form-row.form-row-last").wrapAll(
        "<div class='rs-wrap-coupon'></div>"
    );
    ...
    $(document).ready(function () {
        ...
        $(".menu-button, .close-button, .offwrap").on("click", function () {
            $(".nav-toggle, .menu-ofcn, .close-button, body").toggleClass(
                "off-open"
            );
        });
    ...
    if (navexpander.length) {
        $(".nav-expander, .nav-expander2, .rsoffwrap, .rsoffwrap-close").on(
            "click",
            function (e) {
                e.preventDefault();
        ...
        $(
            "ul.rs_onepage_mobile_menu li a, .right_menu_togle .widget_media_gallery .gallery-item div a"
        ).on("click", function (e) {
            $("body").toggleClass("nav-expanded");
        });
    }
    ...
    if ($(".rs-music").length) {
        document.addEventListener("DOMContentLoaded", function () {
            ...
            var audioElements = document.querySelectorAll(".rs-music");
            audioElements.forEach(function (audio) {
                var playPauseButton =
                    audio.parentElement.querySelector(".playPause");
                var backwardButton =
                    audio.parentElement.querySelector(".backward");
                var forwardButton =
                    audio.parentElement.querySelector(".forward");
                var currentTimeDisplay =
                    audio.parentElement.querySelector(".currentTime");
                var progressContainer = audio.parentElement.querySelector(
                    ".progress-container"
                );
                var progressThumb =
                    progressContainer.querySelector(".progress-thumb");
                if (playPauseButton) {
                    playPauseButton.addEventListener("click", function () {
                        if (currentAudio && currentAudio !== audio) {
                            currentAudio.pause();
                            removePlayingClass(currentAudio);
                        }
                        if (audio.paused) {
                            audio.play();
                            currentAudio = audio;
                            addPlayingClass(currentAudio);
                        } else {
                            audio.pause();
                            currentAudio = null;
                            removePlayingClass(audio);
                        }
                    });
                }
                if (backwardButton) {
                ...
                if (progressContainer && progressThumb) {
                    ...
                    audio.addEventListener("timeupdate", updateProgressThumb);
            ...
            function addPlayingClass(audioElement) {
                if (audioElement && audioElement.parentElement) {
                    audioElement.parentElement.classList.add("playing");
                    audioElement.parentElement.classList.remove("paused");
                }
            }
            function removePlayingClass(audioElement) {
                if (audioElement && audioElement.parentElement) {
                    audioElement.parentElement.classList.remove("playing");
                    audioElement.parentElement.classList.add("paused");
                }
            }
        });
    }
